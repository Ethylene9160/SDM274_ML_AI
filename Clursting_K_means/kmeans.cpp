...
inline double KMeans::distance(my_vector& x, my_vector& y) {
	assert(x.size() == y.size());
	double output = squareDistance(x, y);
	return sqrt(output);
}

double KMeans::loss(coordinate& points) {
...
void KMeans::initMu(my_vector& point) {
	//int N = 999;
	return;
	//srand(time(NULL));
	for (double& px : point) {
...
void KMeans::update(coordinate& points) {
	//清空orderedSample
...
KMeans::KMeans(int k, int dimension) {
	this->k = k;
...
void KMeans::train(coordinate& points, int epochs) {
...
int KMeans::predict(my_vector& singlePoint) {
